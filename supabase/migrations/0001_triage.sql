...
--
-- Everything lives in a dedicated `triage` schema so it is namespaced away from
...
create table if not exists triage.categories (
...
create table if not exists triage.priorities (
  slug       text primary key,
...
create table if not exists triage.contacts (
  id                     uuid primary key default gen_random_uuid(),
  full_name              text not null,
  org                    text,
  is_existing_client     boolean not null default false,
  referred_by_contact_id uuid references triage.contacts (id) on delete set null,
  created_at             timestamptz not null default now(),
...
create table if not exists triage.messages (
  ...
  subject      text,
  body         text not null,
  contact_id   uuid references triage.contacts (id) on delete set null,
  low_signal   boolean not null default false,
  cleaned      jsonb not null default '[]'::jsonb,
  created_at   timestamptz not null default now()
);

create index if not exists messages_received_at_idx on triage.messages (received_at desc);
...
create table if not exists triage.results (
  id             uuid primary key default gen_random_uuid(),
  message_id     text not null references triage.messages (id) on delete cascade,
  summary        text not null,
  category       text not null references triage.categories (slug),
  priority       text not null references triage.priorities (slug),
...
create index if not exists results_message_idx on triage.results (message_id);
create index if not exists results_review_idx on triage.results (needs_review) where needs_review;
...
create or replace view triage.queue as
select
  m.id                as message_id,
  m.received_at,
  m.channel,
...
from triage.messages m
left join triage.results    r  on r.message_id = m.id
left join triage.categories c  on c.slug = r.category
left join triage.priorities p  on p.slug = r.priority
left join triage.contacts   ct on ct.id = m.contact_id
left join triage.contacts   ref on ref.id = ct.referred_by_contact_id;
...
alter table triage.categories enable row level security;
alter table triage.priorities enable row level security;
alter table triage.contacts   enable row level security;
alter table triage.messages   enable row level security;
alter table triage.results    enable row level security;

do $$
begin
  if not exists (select 1 from pg_policies where schemaname = 'triage' and tablename = 'categories' and policyname = 'anon_read_categories') then
    create policy anon_read_categories on triage.categories for select to anon, authenticated using (true);
  end if;
  if not exists (select 1 from pg_policies where schemaname = 'triage' and tablename = 'priorities' and policyname = 'anon_read_priorities') then
    create policy anon_read_priorities on triage.priorities for select to anon, authenticated using (true);
  end if;
  if not exists (select 1 from pg_policies where schemaname = 'triage' and tablename = 'contacts' and policyname = 'anon_read_contacts') then
    create policy anon_read_contacts on triage.contacts for select to anon, authenticated using (true);
  end if;
  if not exists (select 1 from pg_policies where schemaname = 'triage' and tablename = 'messages' and policyname = 'anon_read_messages') then
    create policy anon_read_messages on triage.messages for select to anon, authenticated using (true);
  end if;
  if not exists (select 1 from pg_policies where schemaname = 'triage' and tablename = 'results' and policyname = 'anon_read_results') then
    create policy anon_read_results on triage.results for select to anon, authenticated using (true);
  end if;
end $$;

-- Expose the schema to the Supabase REST API and let anon read the view.
grant usage on schema triage to anon, authenticated;
grant select on all tables in schema triage to anon, authenticated;
alter default privileges in schema triage grant select on tables to anon, authenticated;
